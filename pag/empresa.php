        ...
        if (isset($_REQUEST['empresaCarrito'])) {
            ...
            <div class="products">
                ...
                if (isset($_SESSION['empresaCart'])) {
                    ...
                    // Consultar las propiedades desde la base de datos utilizando los IDs en el carrito
                    $productIds = array_keys($cart);
                    if (!empty($productIds)) {
                        $productIdsString = implode(",", $productIds);
                        if (!empty($productIdsString)) {
                            echo "IDs de propiedades: $productIdsString";
                            // ... Resto de tu código ...
                        } else {
                            echo "La lista de IDs de propiedades está vacía";
                        }
                        ...
                        if ($result->num_rows > 0) {
                            // Iterar sobre las propiedades en el carrito
                            while ($row = $result->fetch_assoc()) {
                                $product_id = $row['id_propiedad'];
                                $product_name = $row['provincia'];
                                $product_description = $row['ubicacion'];
                                $product_price = $row['codigo_postal'];
                                $product_quantity = $cart[$product_id];

                                // Calcular el subtotal por propiedad
                                $subtotal = $product_price * $product_quantity;

                                // Agregar el subtotal al total de dinero
                                $totalMoney += $subtotal;

                                // Mostrar los detalles de la propiedad en el carrito
